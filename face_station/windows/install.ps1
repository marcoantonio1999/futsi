...
$installerValues = @{
    ...
    camera_async_mjpeg_enabled = if ($null -ne $existing.camera_async_mjpeg_enabled) { [bool]$existing.camera_async_mjpeg_enabled } else { $false }
    camera_mjpeg_decode_reduction = if ($existing.camera_mjpeg_decode_reduction) { [int]$existing.camera_mjpeg_decode_reduction } else { 4 }
    ...
    min_det_score = 0.65
    min_face_size = 70
    semantic_reference_filter_enabled = if ($null -ne $existing.semantic_reference_filter_enabled) { [bool]$existing.semantic_reference_filter_enabled } else { $false }
    semantic_reference_model_path = if ($existing.semantic_reference_model_path) { $existing.semantic_reference_model_path } else { "" }
