...
window.fetch=async (input,init)=>{
  ...
  else if(url.includes('/veronica/templates/'))data={templates:[{name:'reclutamiento_primer_mensaje',language:'es',text:'Hola {{1}}. Texto ficticio para revisar la interfaz.',status:'APPROVED',sendable:true,reason:'',parameters:[{key:'body:1',label:'Nombre del destinatario',contact_name:true}]},{name:'pendiente_de_ejemplo',language:'es',text:'Pendiente',status:'PENDING',sendable:false,reason:''}],next_cursor:''};
  else if(url.includes('/veronica/upload/'))data={media_token:'mock-upload'};
  else if(url.includes('/veronica/send/')){const body=JSON.parse(String(init?.body));(window as any).__lastVeronicaSend=body;sent.push({id:sent.length+61,body:body.kind==='text'?body.body:body.kind==='document'?'[PDF] Prueba simulada':'[Plantilla] Prueba simulada',direction:'outbound',created_at:new Date().toISOString(),status:'accepted',error_codes:[]});data={conversation_id:1,status:'accepted',detail:'',message_id:'mock'};}
