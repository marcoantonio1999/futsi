...
export function WhatsAppTemplatesPanel({ token, address, onOpenCollections }: { token: string; address: string; onOpenCollections?: () => void }) {
  ...
  return <div className="grid gap-4">
    ...
    {catalog && <>
      ...
      {visible.map(t => <article key={`${t.id}:${t.name}:${t.language}`} className="comm-panel"><header className="comm-section-heading"><div><h3>{t.name}</h3><p>{t.language} · {t.category}</p></div><span className={`comm-badge ${t.status.toUpperCase() === "APPROVED" ? "green" : "amber"}`}>{statuses[t.status.toUpperCase()] || t.status || "Estado desconocido"}</span></header>
        <div className="p-4 grid gap-3">{t.components.map((component, i) => <div key={i}><small className="comm-muted">{component.type}{component.format ? ` · ${component.format}` : ""}</small>{component.text && <p className="whitespace-pre-wrap break-words text-sm">{component.text}</p>}{component.buttons.map((button, j) => <span key={j} className="inline-block rounded-md border border-zinc-300 px-3 py-2 text-sm mr-2">{button.text || button.type}</span>)}</div>)}{t.rejected_reason && <p className="comm-error">Motivo reportado: {t.rejected_reason}</p>}<p className="comm-muted">Vista de la plantilla; las variables se completan al enviar. Este catálogo no envía mensajes.</p></div>
      </article>)}
