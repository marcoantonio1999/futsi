    ...
    {!job ? <>
      ...
      <section className="bulk-card"><h3>2. ¿A quiénes se enviará?</h3><p>Solo números de México de 10 dígitos. No escribas código de país. Hasta 1,000 números por lote.</p>
        ...
        {mode === 'text' ? <label>Números separados por comas<textarea disabled={busy} rows={4} value={text} maxLength={25000} placeholder="5512345678, 5587654321" onChange={e => { setText(e.target.value); invalidate(); }} /></label> : <>
          ...
          </label><p>Excel/CSV: una columna llamada “teléfono”, o selecciona la columna al cargar. TXT: números separados por comas o renglones.</p>
        ...
        {review && !review.needs_column && <div className="bulk-review" aria-live="polite"><h4>{review.count} números válidos · {review.duplicates} duplicados excluidos · {review.invalid.length} inválidos excluidos</h4>
          ...
          <div className="bulk-numbers">{review.phones.slice(reviewPage*50, reviewPage*50+50).map(p => <span key={p}>{p}</span>)}</div>
      ...
      <button className="primary bulk-prepare" disabled={busy || !selected?.sendable || !review?.count || review.needs_column || selected.parameters.some(p => !parameters[p.key]?.trim())} onClick={() => void action(async () => {
        const saved = await post<Job>('create', { request_id: requestId.current, channel, name: selected?.name, language: selected?.language, parameters, phones: review?.phones });
      ...
      <h4>Destinatarios del lote</h4><div className="bulk-table-wrap"><table><thead><tr><th>Número</th><th>Estado</th><th>Qué ocurrió</th></tr></thead><tbody>{job.recipients?.map(r => <tr key={r.id}><td>{r.phone}</td><td><span className={`bulk-state ${r.status}`}>{labels[r.status] || r.status}</span></td><td>{r.detail || '—'}</td></tr>)}</tbody></table></div>
