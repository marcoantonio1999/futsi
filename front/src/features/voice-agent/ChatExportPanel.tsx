...
export function ChatExportPanel({ token, scopeQuery, conversations }: {
  ...
  return <section className="comm-panel chat-export-panel">
    <header className="comm-section-heading"><div><h3>Exportar historial de WhatsApp</h3><p>Descarga todos los chats de la sede y el número seleccionados arriba.</p></div><FileSpreadsheet size={22} aria-hidden="true" /></header>
    <div className="chat-export-body">
      <div className="chat-export-copy">
        <h4>El Excel incluye dos pestañas</h4>
        <ul>
          <li><strong>Conversaciones:</strong> número de atención, sede, teléfono, nombre del contacto y cantidades de mensajes recibidos y enviados.</li>
          <li><strong>Mensajes:</strong> el historial completo, una fila por mensaje, con fecha, dirección, texto y estado.</li>
        </ul>
      ...
      <aside className="chat-export-summary" aria-label="Resumen de la exportación">
        <div><UsersRound size={18} aria-hidden="true" /><span>Contactos</span><strong>{contactCount.toLocaleString("es-MX")}</strong></div>
        <div><MessageSquareText size={18} aria-hidden="true" /><span>Mensajes</span><strong>{messageCount.toLocaleString("es-MX")}</strong></div>
