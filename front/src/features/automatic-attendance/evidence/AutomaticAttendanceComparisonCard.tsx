...
import type { FaceComparison } from "../report/AutomaticAttendanceReport";
import { comparisonTimeText } from "../report/model";
...
export function AutomaticFaceComparisonCard({
  ...
  const timeText = comparisonTimeText(comparison);
  const cameraLabel = comparison.source_camera_label || comparison.source_camera_id || "";
    ...
    <article className={`rounded-md border-2 ${tone} p-3`}>
      ...
      <div className="mt-3 flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          {cameraLabel ? <p className="mt-1 text-xs font-semibold text-violet-700">Camara {cameraLabel.replace(/^camara\s+/i, "")}</p> : null}
          {timeText ? <p className="mt-1 text-xs font-semibold text-zinc-700">Tiempo {timeText}</p> : null}
        </div>
