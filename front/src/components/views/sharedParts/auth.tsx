...
export function LoginForm({ onLogin, className = "" }: { onLogin: (token: string, user: User) => void; className?: string }) {
  ...
  return (
    <form onSubmit={submit} className={`w-full max-w-sm rounded-md border border-zinc-200 bg-white p-6 text-zinc-950 shadow-sm dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-50 ${className}`} data-testid="login-form">
      <div className="flex items-center gap-3">
        <img className="h-12 w-12 rounded-md object-cover" src="./favicon.png" alt="Futsi" />
        <div>
          <p className="text-sm font-medium text-emerald-700 dark:text-emerald-300">Futsi Mini ERP</p>
      ...
      <button
        ...
        className="mt-5 flex w-full items-center justify-center gap-2 rounded-md bg-zinc-950 px-4 py-2.5 text-sm font-medium text-white disabled:opacity-60 dark:bg-emerald-600 dark:hover:bg-emerald-500"
        disabled={loading}
      >
        <Shield size={16} />
