...
export function SportsMonthlyExams({ data, canEdit, onSaveAssessment }: { data: AppData; canEdit: boolean; onSaveAssessment: (payload: unknown) => Promise<void> }) {
  ...
  return <div data-testid="sports-monthly-exams">
    <header className="sports-header"><div><p className="sports-eyebrow">Deportivo / Evaluación</p><h2>Examen mensual</h2><p>Consulta y registra la evaluación de cada alumno por mes.</p></div></header>
    <fieldset className="sports-exam-filters" disabled={busy}><legend className="sr-only">Filtrar exámenes</legend><TextInput label="Mes de evaluación" type="month" required value={month} onChange={event => { if (event.target.value) setMonth(event.target.value); }} /><SelectInput label="Sede" value={site} onChange={event => setSite(event.target.value)}><option value="">Todas las sedes</option>{data.sites.map(row => <option key={row.id} value={row.id}>{row.name}</option>)}</SelectInput><TextInput label="Buscar alumno o grupo" placeholder="Escribe un nombre" value={query} onChange={event => setQuery(event.target.value)} /></fieldset>
    <div className="sports-summary" aria-label="Evaluación del mes seleccionado"><div className="done"><span>Evaluados este mes</span><strong>{evaluated.length}<small> / {students.length}</small></strong></div><div className="pending"><span>Pendientes de examen</span><strong>{students.length - evaluated.length}</strong></div><div className="upcoming"><span>Promedio del mes</span><strong>{evaluated.length ? Math.round(evaluated.reduce((sum, exam) => sum + Number(exam.overall_rating), 0) / evaluated.length) : "—"}<small> / 100</small></strong></div></div>
...
function ExamForm({ studentId, month, current, onSave, onBusyChange }: { studentId: number; month: string; current?: StudentAssessment; onSave: (payload: unknown) => Promise<void>; onBusyChange: (busy: boolean) => void }) {
  ...
  async function submit(event: FormEvent) {
    ...
    } catch (reason) { setError(reason instanceof Error ? reason.message : "No se pudo guardar el examen."); }
  ...
  return <form className="sports-surface sports-exam-form" onSubmit={submit}>
    <header className="sports-section-header"><div><h3>{current ? "Actualizar examen" : "Registrar examen"}</h3><p>Califica cada habilidad de 0 a 100.</p></div><span className={`sports-badge ${current ? "done" : "pending"}`}>{current ? "Registrado" : "Pendiente"}</span></header>
    <fieldset disabled={busy}><legend className="sr-only">Calificaciones del examen mensual</legend><div className="sports-skills">{skills.map(([key, label]) => <label key={key}><span>{label}</span><input type="number" min={0} max={100} step={1} required inputMode="numeric" placeholder="0–100" value={scores[key]} onChange={event => { setScores(previous => ({ ...previous, [key]: event.target.value })); setMessage(""); }} /><div className="sports-skill-bar" aria-hidden="true"><span style={{ width: `${Math.min(100, Math.max(0, Number(scores[key]) || 0))}%` }} /></div></label>)}</div><label className="sports-notes">Observaciones del examen<textarea rows={3} placeholder="Avances y habilidades que debe trabajar el alumno" value={notes} onChange={event => { setNotes(event.target.value); setMessage(""); }} /></label>
    ...
    <footer><button className="sports-button primary" disabled={busy}><Save size={16} />{busy ? "Guardando examen…" : current ? "Guardar cambios" : "Guardar examen mensual"}</button></footer></fieldset>
  </form>;
}
