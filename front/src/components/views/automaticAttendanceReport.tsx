...
import { attendanceSummary, buildReportGroups, resultForSession, type AutomaticSessionResult, type ReportType } from "./automaticAttendanceReportModel";

export type { AutomaticSessionResult, FaceComparison } from "./automaticAttendanceReportModel";
    ...
    <section className="overflow-hidden rounded-md border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
      <div className="border-b border-zinc-200 p-4 dark:border-zinc-800">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wide text-blue-700 dark:text-blue-300">Reporte automatico</p>
            <h2 className="mt-1 text-lg font-semibold text-zinc-950 dark:text-zinc-50">Sesiones procesadas y pase de lista</h2>
        ...
        <table className="min-w-full border-collapse text-left text-sm text-zinc-900 dark:text-zinc-100">
          ...
          <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
            {reportGroups.map((group) => {
              ...
              const offRosterCount = results.reduce((sum, item) => sum + (item.off_roster?.length ?? 0), 0);
              ...
              return (
                <tr key={group.id} className="bg-white transition hover:bg-blue-50/60 dark:bg-zinc-950 dark:hover:bg-blue-950/20">
                  ...
                  <td className="px-4 py-3">
                    {offRosterCount ? (
                      <span className="inline-flex rounded-md border border-red-200 bg-red-50 px-2 py-1 text-xs font-semibold text-red-800">{offRosterCount} detectado{offRosterCount === 1 ? "" : "s"}</span>
                    ) : (
