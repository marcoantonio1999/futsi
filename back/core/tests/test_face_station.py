...
def test_bootstrap_is_scoped_to_station_site(api_client, station_context):
    ...
    paid_charge = make_charge(student=own_student, site=station_context["site"])
    ...
    assert payload["monthly_payments"] == [
        {
            "person_key": f"student:{own_student.id}",
            "month": payment.paid_at.strftime("%Y-%m"),
            "payment_count": 1,
            "amount": "325.00",
            "last_paid_at": payment.paid_at.isoformat(),
        }
    ]
